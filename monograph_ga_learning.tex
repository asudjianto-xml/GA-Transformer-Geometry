...
change.}  Scalar curvature magnitude may change by only 10--20\%, but
the \emph{direction} of curvature can shift substantially, indicating
...
$G_{ij} = v_i^\top v_j$ measures spectral concentration.  When the
model succeeds, reasoning paths are spectrally concentrated (low
effective rank); when it fails, paths scatter more diffusely.  High
$\kappa_G$ with low $\mathcal{C}$ is a \emph{warning sign}: the
apparent consistency may be a metric artefact rather than genuine
agreement.
\end{finding}
...
\begin{finding}
\textbf{Rotation is the mechanism of semantic steering.}
Across empirical studies, the central finding is that context
influence in language models is a rotational phenomenon.  The Cayley
bivector provides a unified representation: its magnitude detects
vibing, its direction diagnoses the semantic category of context
influence, and its associated rotation enables active intervention.
Ablation studies show that random rotations catastrophically fail
(0\% context-following), while learned rotations succeed (83\%),
and scaling alone has no effect --- confirming that the antisymmetric
(bivector) component carries the semantically meaningful signal.
\end{finding}
