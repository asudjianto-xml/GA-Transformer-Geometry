...
Concretely, the final hidden state $H^{(L-1, t)}$ at the last token
position is projected to a score for every word in the vocabulary via
a learned linear map $W_{\mathrm{head}} \in \mathbb{R}^{V \times p}$:
\[
  \mathrm{logits} = W_{\mathrm{head}} \, H^{(L-1, t)}
\]
where $V$ is the vocabulary size (e.g., $V = 151{,}936$ for
Qwen2.5-7B).  These scores are called \textbf{logits}; higher values
mean higher probability.  Applying a softmax converts logits to a
probability distribution over the vocabulary, and the
highest-probability token is selected as the prediction.
...
To generate a full sentence, this process repeats: the predicted token
is appended to the input, and the model runs again to predict the next
token, one step at a time.
