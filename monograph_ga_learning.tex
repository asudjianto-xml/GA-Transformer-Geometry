...
the final hidden state must point in the ``right direction'' for the
correct prediction to win.  In Chapter~8, we observe that the last few
layers have highly peaked eigenvalue spectra (condition numbers
$> 10^3$, effective rank $< 10$): the metric factor $P^{(l)}$
aggressively amplifies a small number of directions and suppresses the
rest.  This is the grade-0 mechanism for ``choosing'' a prediction ---
funnelling the hidden state toward the direction of the correct
vocabulary vector.
