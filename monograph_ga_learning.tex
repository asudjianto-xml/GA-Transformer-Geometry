...
\medskip
\textbf{What this tells you.}
A peak rotation of $4.28$ radians (about $245^\circ$) means the first layer nearly reverses the hidden-state direction --- a massive reorientation that encodes token identities into the model's internal coordinate system. The grade-2 norm ($5.30$) exceeding the grade-0 norm ($2.24$) tells us the network rotates more than it stretches, on average. However, as the plot below shows, the balance shifts toward grade-0 in later layers, where selective amplification matters more than reorientation.
...
plane information.  Scalar summaries --- the size of a rotation
($\|U - I\|_F$), the size of a commutator ($\|[A_i, A_j]\|_F$), or the
size of holonomy ($\|\Omega - I\|_F$) --- tell you \emph{how much}.
The GA equivalents tell you \emph{how much and in which plane}.
\end{gabox}
...
\emph{amplify} that direction relative to competitors.  This is why
grade-0 controls the output: modifying eigenvalues (grade-0) has a 48\%
effect on dependency while modifying the rotor (grade-2) has only a
9\% effect.

Both grades are essential, but they serve different roles: rotation
\emph{steers}, stretching \emph{selects}.
\end{keyidea}
...
suppressed.  This is the metric factor implementing the model's ``decision'' about
which information to propagate to the final prediction.  The reason is
architectural: the logit for word $w$ is the dot product
$W_{\mathrm{head}}[w] \cdot H^{(L-1)}$, so the final hidden state
must point toward the correct vocabulary vector.  The peaked spectrum
in late layers is the grade-0 mechanism that achieves this alignment
--- amplifying the target direction while suppressing competitors.
\end{finding}
...
Early layers have near-uniform eigenvalue spectra (effective rank close to $k$, low condition number): they treat all directions roughly equally. The final layers show condition numbers exceeding $10^3$ and effective ranks below~10, funnelling almost all energy into a handful of directions. This is the grade-0 mechanism for ``choosing'' a prediction: amplify the winning directions, suppress everything else.
...
\medskip
\textbf{What this tells you.}
The commutator norm $0.571$ means these two layer bivectors are significantly non-commuting --- applying their rotations in different orders produces meaningfully different outcomes. The zero self-commutator and machine-precision Jacobi residual are sanity checks confirming the algebra is consistent. In the heatmap below, look for off-diagonal blocks of high commutator norm: these identify groups of layers whose rotations interfere with each other and therefore perform jointly non-trivial computation.
...
\medskip
\textbf{What this tells you.}
High curvature at the final layers means the rotor field is most ``twisted'' where the model makes its prediction. Transporting a vector around a layer-token loop at this point returns it substantially rotated, indicating that the network's layer and token transformations are strongly coupled. This is a signature of active computation: the model is not just passing information through but actively reshaping it in a way that depends jointly on layer depth and token position.
...
\medskip
\textbf{What this tells you.}
The accumulated capacity $C_{\mathrm{acc}} = 3771.3$ sums the non-commutativity across all adjacent layer pairs --- it is the network's total ``geometric work.'' The concentration of $0.205$ (on a 0-to-1 scale, where 1 means all capacity sits in a single layer pair) tells us computation is moderately distributed: no single transition dominates, but the load is far from uniform. The plot below reveals which transitions carry disproportionate weight, identifying the computational bottlenecks.
...
\medskip
\textbf{What this tells you.}
The total dependency $D_{\mathrm{total}} = 1.74$ measures the output's overall gradient sensitivity across all layers. The entropy of $2.61$ (out of a maximum of $\ln(28) \approx 3.33$ for 28 layers) indicates that sensitivity is spread across many layers but not uniformly. The surprise is that the top effective-work layers are 0, 1, and 2: the model's output depends most on the rotations in its very first layers, where the geometric reorientation is largest.
